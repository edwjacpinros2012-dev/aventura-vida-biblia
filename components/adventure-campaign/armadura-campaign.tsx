...
function AdventureStage({ stage, onComplete, avatarKey, nickname }: { stage: CampaignStage; onComplete: (elapsedSeconds: number) => void; avatarKey: string; nickname: string }) {
  ...
  return (
    <div className="rounded-[2rem] border border-ink/10 bg-white p-3 shadow-lift sm:p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 px-1">
        ...
        <div className="flex items-center gap-2"><span className="rounded-full bg-sun/30 px-3 py-2 text-xs font-black text-ink">✦ {collected.length}/{stage.tokens.length}</span><span className="hidden rounded-full bg-sky px-3 py-2 text-xs font-black text-ink sm:inline">{nickname} · Explorador</span></div>
      </div>
      ...
      <div className="relative min-h-[290px] overflow-hidden rounded-[1.5rem] border-4 border-white bg-[linear-gradient(180deg,#86ddff_0%,#c8f2ff_53%,#def7d3_54%,#a7dc83_100%)] shadow-inner sm:min-h-[380px]" aria-label={`Nivel jugable: ${stage.title}`}>
        ...
        <div className="absolute z-20 grid h-12 w-10 place-items-center transition-[left,top] duration-75" style={{ left: `${view.x}%`, top: `${view.y}%` }} aria-label={`${nickname}, aventurero`}><CharacterAvatar avatarKey={avatarKey} fallback={<div className="relative grid h-10 w-8 place-items-center rounded-t-[1.2rem] rounded-b-lg border-2 border-ink bg-coral shadow-card"><span className="text-base">✦</span><span className="absolute -right-2 bottom-0 grid h-5 w-5 place-items-center rounded bg-white text-[9px]">📖</span></div>} alt={`Personaje de ${nickname}`} imageSizes="48px" variant="full" className="grid h-12 w-10 place-items-center" /></div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      ...
      <div className="mt-8" aria-live="polite">
        ...
        {screen === "playing" && <AdventureStage key={stage.id} stage={stage} onComplete={completeStage} avatarKey={identity.avatarKey} nickname={identity.nickname} />}
