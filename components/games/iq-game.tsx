...
export function IqGame() {
  ...
  const [round, setRound] = useState(0); const [index, setIndex] = useState(0); const [selected, setSelected] = useState<number | null>(null); const [correct, setCorrect] = useState(0);
  const roundPuzzles = useMemo(() => [...puzzles].sort((first, second) => (first.prompt.length + round) % 7 - (second.prompt.length + round) % 7), [round]);
  const current = roundPuzzles[index];
  const choose = (value: number) => { if (selected === null) setSelected(value); };
  const next = () => { const nextCorrect = correct + (selected === current.correct ? 1 : 0); if (index + 1 >= roundPuzzles.length) { setCorrect(nextCorrect); finish({ points: 120 + nextCorrect * 65, xp: 70 + nextCorrect * 15 }); return; } setCorrect(nextCorrect); setIndex((value) => value + 1); setSelected(null); };
  const reset = () => { setRound((value) => value + 1); setIndex(0); setSelected(null); setCorrect(0); resetCompletion(); };
  const points = 120 + correct * 65; const xp = 70 + correct * 15;
  if (result) return <VictoryPanel title="¡Tu mente encontró las pistas!" description={`Respondiste ${correct} de ${roundPuzzles.length} retos de lógica. ¡Sigue haciendo buenas preguntas!`} result={result} points={points} xp={xp} onPlayAgain={reset} />;
  return <div className="mx-auto mt-6 max-w-3xl rounded-[1.7rem] bg-white p-5 shadow-card sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><span className="rounded-full bg-violet/10 px-3 py-1.5 text-xs font-black uppercase tracking-wide text-violet">Reto {index + 1} de {roundPuzzles.length} · ronda {round + 1}</span><span className="text-sm font-bold text-ink/55">{correct} correctas</span></div><p className="mt-7 text-xs font-black uppercase tracking-[.18em] text-violet">{current.label}</p><h2 className="mt-3 font-display text-3xl font-black leading-tight text-ink">{current.prompt}</h2><div className="mt-7 grid gap-3 sm:grid-cols-2">{current.options.map((option, optionIndex) => <button key={option} type="button" disabled={selected !== null} onClick={() => choose(optionIndex)} className={`min-h-16 rounded-2xl border-2 p-4 text-left text-sm font-bold outline-none transition-all focus-visible:ring-4 focus-visible:ring-violet/25 ${selected === null ? "border-ink/5 bg-sky/40 hover:border-violet" : optionIndex === current.correct ? "border-leaf bg-leaf/10 text-[#147558]" : optionIndex === selected ? "border-coral bg-coral/10 text-coral" : "border-ink/5 text-ink/40"}`}>{option}</button>)}</div>{selected !== null && <div className={`mt-5 rounded-2xl p-4 text-sm leading-6 ${selected === current.correct ? "bg-leaf/10 text-[#147558]" : "bg-coral/10 text-[#af3f3b]"}`}><strong>{selected === current.correct ? "¡Buena lógica!" : "Una pista más:"}</strong><p className="mt-1 font-semibold">{current.explanation}</p></div>}{selected !== null && <button type="button" onClick={next} className="mt-6 rounded-xl bg-ink px-5 py-3 text-sm font-extrabold text-white hover:bg-violet focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-violet/25">{index + 1 === roundPuzzles.length ? "Ver recompensa" : "Siguiente reto"}</button>}</div>;
}
