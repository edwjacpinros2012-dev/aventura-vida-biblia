...
const verses = [
  { reference: "Filipenses 4:13", text: "Todo lo puedo en Cristo que me fortalece", words: ["Todo", "lo", "puedo", "en", "Cristo", "que", "me", "fortalece"] },
  { reference: "Salmo 119:105", text: "Lámpara es a mis pies tu palabra", words: ["Lámpara", "es", "a", "mis", "pies", "tu", "palabra"] },
];
...
export function VerseGame() {
  ...
  const shuffledWords = [...verse.words].sort((first, second) => (first.length + verseIndex) % 7 - (second.length + verseIndex) % 7);
  useEffect(() => { saveGameProgress("verse-learn", { step, reference: verse.reference }); }, [saveGameProgress, step, verse.reference]);
  const reset = () => { setVerseIndex((value) => (value + 1) % verses.length); setStep(0); setAnswer(""); setSelectedWords([]); setMessage(""); resetCompletion(); };
  const checkWord = () => {
    if (answer.trim().toLocaleLowerCase() === verse.words.at(-1)?.toLocaleLowerCase()) { setMessage(""); setStep(2); }
