...
export function VictoryPanel({ title = "¡Aventura completada!", description, result, points, xp, onPlayAgain, actionLabel = "Jugar otra vez", secondaryAction }: { title?: string; description: string; result: RewardResult | null; points: number; xp: number; onPlayAgain: () => void; actionLabel?: string; secondaryAction?: { label: string; onAction: () => void } }) {
  const { identity } = usePlayerProgress();
  return <div className="mx-auto mt-6 max-w-2xl overflow-hidden rounded-[1.7rem] bg-[#fff5cf] p-6 text-center shadow-card sm:p-8"><div className="mx-auto flex w-fit items-center gap-3 rounded-2xl bg-white/70 px-3 py-2 text-left shadow-card"><CharacterAvatar avatarKey={identity.avatarKey} fallback={avatarFallbackFor(identity.avatarKey)} alt={`Avatar de ${identity.nickname}`} imageSizes="40px" className="grid h-10 w-10 place-items-center rounded-xl bg-sun text-xl" /><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#a66d00]">Recompensa de</p><p className="text-sm font-extrabold text-ink">{identity.nickname}</p></div></div><span className="mx-auto mt-4 grid h-16 w-16 place-items-center rounded-[1.5rem] bg-sun text-4xl shadow-card">🏆</span><p className="mt-5 text-xs font-black uppercase tracking-[.2em] text-[#a66d00]">Nivel completado</p><h2 className="mt-2 font-display text-3xl font-black text-ink">{title}</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-ink/70">{description}</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-white p-4"><p className="font-display text-2xl font-black text-violet">+{points}</p><p className="mt-1 text-xs font-bold text-ink/55">Puntos</p></div><div className="rounded-xl bg-white p-4"><p className="font-display text-2xl font-black text-leaf">+{xp}</p><p className="mt-1 text-xs font-bold text-ink/55">XP</p></div></div>{result && result.newAchievementIds.length > 0 && <div className="mt-4 rounded-xl bg-white/80 p-4 text-left"><p className="text-xs font-black uppercase tracking-[.16em] text-violet">Nuevo logro</p>{result.newAchievementIds.map((id) => <p className="mt-2 text-sm font-bold text-ink" key={id}>{achievementDetails[id].icon} {achievementDetails[id].title}</p>)}</div>}<div className="mt-6 flex flex-wrap justify-center gap-3"><button type="button" onClick={onPlayAgain} className="rounded-xl bg-ink px-5 py-3 text-sm font-extrabold text-white outline-none transition-colors hover:bg-violet focus-visible:ring-4 focus-visible:ring-violet/25">{actionLabel}</button>{secondaryAction && <button type="button" onClick={secondaryAction.onAction} className="rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-violet outline-none ring-1 ring-violet/15 transition-colors hover:bg-violet/10 focus-visible:ring-4 focus-visible:ring-violet/25">{secondaryAction.label}</button>}</div></div>;
}
