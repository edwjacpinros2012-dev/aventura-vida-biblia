    ...
    <header className="sticky top-0 z-50 border-b border-ink/5 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <Link href="/perfil" className="grid h-10 w-10 place-items-center rounded-full bg-sun text-lg outline-none transition-transform hover:scale-105 focus-visible:ring-4 focus-visible:ring-violet/25" aria-label="Ver perfil de demostración">🦊</Link>
      ...
      {open && (
        <nav className="border-t border-ink/5 bg-white px-4 pb-4 pt-2 shadow-card lg:hidden" aria-label="Navegación móvil">
          <div className="mx-auto grid max-w-7xl gap-1">
            ...
            <Link href="/perfil" onClick={() => setOpen(false)} className="mt-1 rounded-xl bg-ink px-4 py-3 text-center font-extrabold text-white">Ver perfil de muestra</Link>
          </div>
        </nav>
      )}
    </header>
