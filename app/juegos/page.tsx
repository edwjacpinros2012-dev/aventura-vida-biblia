...
export default async function GamesPage({ searchParams }: { searchParams: Promise<{ category?: string; filter?: string }> }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 pb-10 pt-12 sm:px-6 sm:pt-16 lg:px-8">
      ...
      <p className="mt-10 rounded-2xl bg-sky px-5 py-4 text-sm leading-6 text-ink/70"><span className="font-extrabold text-ink">13 juegos listos para jugar:</span> completa retos para guardar puntos, XP, racha, logros, juegos terminados y versículos aprendidos en este navegador. Las cuentas y la validación de servidor llegarán en una fase posterior.</p>
    </section>
  );
}
